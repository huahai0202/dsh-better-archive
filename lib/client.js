  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && !document.getElementById(STYLE_ID)) {
      ...
      styleEl.textContent = [
        ...
        // Archive toast stack container: holds multiple stacked or expanded toasts.
        '._dsh_ba_toast_stack { position: fixed; top: 36px; left: 50%; z-index: 1100; transform: translateX(-50%); display: flex; flex-direction: column; align-items: center; padding: 4px; pointer-events: auto; }',
        '._dsh_ba_toast { position: relative; display: flex; align-items: center; gap: 10px; width: max-content; max-width: min(640px, calc(100vw - 48px)); height: 42px; box-sizing: border-box; padding: 0 10px 0 14px; border-radius: 14px; border: 1px solid var(--dsw-alias-border-l3, rgba(255, 255, 255, 0.08)); background: var(--dsw-specific-menu, var(--dsw-alias-bg-layer-3)); color: var(--dsw-alias-label-primary); box-shadow: var(--dsw-elevation-prominent, var(--dsw-shadow-lv3)); font-size: 14px; line-height: 22px; transform-origin: top center; transition: margin-top .24s cubic-bezier(0.16, 1, 0.3, 1), transform .24s cubic-bezier(0.16, 1, 0.3, 1), opacity .2s ease, box-shadow .2s ease; }',
        ...
        '._dsh_ba_toast_entering { animation: _dsh-ba-toast-in .18s ease-out; }',
    ...
    /**
     * One archive toast card inside the stack. Supports:
     * - Stacking: behind items are scaled down and offset so their bottom edge peeks out.
     * - Expansion: when hovered/expanded, items fan out vertically with clean spacing.
     * - Individual hold/dismiss lifecycle, pausing while the stack is hovered.
     */
    function ArchivedToastItem(props) {
      ...
      var itemStyle = {}
      if (isExpanded) {
        itemStyle.zIndex = 30 - index
        itemStyle.transform = 'scale(1)'
        itemStyle.marginTop = index === 0 ? 0 : 10
        itemStyle.opacity = 1
        ...
        if (index === 0) {
          ...
          itemStyle.zIndex = 29
          itemStyle.transform = 'scale(0.96)'
          itemStyle.marginTop = -34
          itemStyle.opacity = 0.95
          itemStyle.pointerEvents = 'auto'
        } else if (index === 2) {
          itemStyle.zIndex = 28
          itemStyle.transform = 'scale(0.92)'
          itemStyle.marginTop = -34
          itemStyle.opacity = 0.85
          itemStyle.pointerEvents = 'auto'
        } else {
          itemStyle.zIndex = 20
          itemStyle.transform = 'scale(0.88)'
          itemStyle.marginTop = -42
          itemStyle.opacity = 0
          itemStyle.pointerEvents = 'none'
        }
      }

      var className = '_dsh_ba_toast'
      if (index === 0 && !leaving) className += ' _dsh_ba_toast_entering'
      if (leaving) className += ' _dsh_ba_toast_leaving'
    ...
    /**
     * Always-mounted overlay host: watches the archive set and displays stacked toasts
     * when sessions are archived. Collapses into stacked cards (showing the peek edge
     * beneath) and expands on mouse hover so any individual item can be inspected or undone.
     */
    function ArchiveToastHost(props) {
      ...
      return createElement('div', {
        className: hovered ? '_dsh_ba_toast_stack _dsh_ba_toast_stack_expanded' : '_dsh_ba_toast_stack',
        onMouseEnter: function () { setHovered(true) },
        onMouseLeave: function () { setHovered(false) },
      },
        toasts.map(function (item, index) {
