...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ArchivedSection(props) {
      ...
      /** Every action converges here: clear the busy flag and resync the pending-deletion mirror. */
      function settle() {
        setBusy(false)
        refreshPendingDeletions()
      }
      ...
      function act(path, payload) {
        ...
        return fetch(path, {
          ...
          .then(function (res) {
            // A scheduled (restart-pending) deletion is known the moment the
            // response lands - merge it into the shared cache so the row hides
            // immediately and a remount does not flash it back.
