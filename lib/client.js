...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** How long an untouched archive toast stays up before it dismisses itself. */
    var TOAST_HOLD_MS = 6000
    ...
    if (typeof document !== 'undefined' && !document.getElementById(STYLE_ID)) {
      ...
      styleEl.textContent = [
        ...
        // Destructive accent for the delete-all action (DSH ships no danger Button
        // variant; this reuses the token its own Menu `danger` row hovers with).
        '._dsh_ba_danger { color: var(--dsw-alias-state-error-primary); border-color: var(--dsw-alias-state-error-primary); }',
        '._dsh_ba_danger:hover:not(:disabled) { background: var(--dsw-alias-interactive-bg-hover-danger); }',
        ...
        // Archive toast. DSH's own Toast renders text + icon only (no action slot),
        // so the bar is plugin-built but takes DSH's menu surface, its elevation and
        // its inverted "solid" pill recipe, which keeps it readable in both themes.
        '._dsh_ba_toast { position: fixed; top: 40px; left: 50%; z-index: 1100; display: flex; align-items: center; gap: 10px; width: max-content; max-width: min(640px, calc(100vw - 48px)); padding: 8px 10px 8px 14px; border-radius: 14px; background: var(--dsw-specific-menu, var(--dsw-alias-bg-layer-3)); color: var(--dsw-alias-label-primary); box-shadow: var(--dsw-elevation-prominent, var(--dsw-shadow-lv3)); font-size: 14px; line-height: 22px; transform: translateX(-50%); animation: _dsh-ba-toast-in .16s ease-out; }',
        '._dsh_ba_toast_icon { flex: none; color: var(--dsw-alias-label-tertiary); }',
        '._dsh_ba_toast_text { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }',
        ...
        '@keyframes _dsh-ba-toast-in { from { opacity: 0; transform: translate(-50%, -6px); } to { opacity: 1; transform: translateX(-50%); } }',
      ].join('\n')
    ...
    /** Shared layout: an even toolbar row where every control is 36px tall. */
    var TOOLBAR_CELL = { flex: '1 1 0', width: 0, minWidth: 0 }
    ...
    /**
     * The archive toast: what DSH does not offer. Its own `Toast` primitive renders
     * text and an icon and nothing else, so the action bar is plugin-built, on DSH's
     * menu surface with DSH's own tokens. Self-dismisses, and holds while hovered so
     * a slow click cannot lose the undo.
     */
    function ArchivedToast(props) {
      var hoverState = React.useState(false)
      var hovered = hoverState[0]
      var setHovered = hoverState[1]
      // Held in a ref: the parent re-creates its callbacks on every render, and an
      // identity dependency would restart the hold timer each time.
      var closeRef = React.useRef(props.onClose)
      closeRef.current = props.onClose

      React.useEffect(function () {
        if (hovered) return undefined
        var timer = setTimeout(function () { closeRef.current() }, TOAST_HOLD_MS)
        return function () { clearTimeout(timer) }
      }, [hovered])

      return createElement('div', {
        className: '_dsh_ba_toast',
        role: 'status',
        onMouseEnter: function () { setHovered(true) },
        onMouseLeave: function () { setHovered(false) },
      },
        createElement(Icon, { name: 'IconArchiveOutline20', size: 16, className: '_dsh_ba_toast_icon' }),
        createElement('span', { className: '_dsh_ba_toast_text' }, props.text),
    ...
    /**
     * Always-mounted overlay host: DSH's archive action lives in its own session
     * menu, so the plugin learns about an archive by watching the archive set rather
     * than by owning the gesture. The first observation only adopts the existing set
     * — otherwise every page load would toast about long-archived chats.
     */
    function ArchiveToastHost(props) {
      ...
      React.useEffect(function () {
        ...
        if (added.length > 0) setToast({ id: added[0], count: added.length })
        // `signature` is the dependency: the archive set arrives as a fresh array on
        // every snapshot, so identity comparison would fire on unrelated updates.
      }, [signature])

      if (toast === null) return null
      var summary = list && list.byId ? list.byId[toast.id] : undefined

      return createElement(ArchivedToast, {
        text: tr('toastArchived', { title: (summary && summary.displayTitle) || toast.id }),
      ...
      return createElement(
        'div',
        {
          style: {
            ...
            width: '100%', maxWidth: isPage ? 900 : 760, boxSizing: 'border-box',
            color: 'var(--dsw-alias-label-primary, inherit)',
          },
        },
        ...
        createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 } },
          createElement('div', { className: '_dsh_ba_search', style: TOOLBAR_CELL },
