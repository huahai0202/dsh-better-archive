...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        isSidebar
          ? // Controls on one wrapping row: the search grows to a capped width
            ...
            createElement('div', {
              ...
              createElement('div', { className: '_dsh_ba_search', style: { flex: '1 1 120px', minWidth: 0, maxWidth: 200 } },
    ...
    function apply(ctx) {
      ...
      // The toast's "view" action opens the right-Sidebar archive tab. The
      // service is read defensively; `openTab` only runs from a toast, whose
      // view button is itself shown while a session is open, so a failure here
      // is a vanished surface and just logs.
       ...
       * injection — it kept working exactly as before this feature — so the
       * toast host must degrade to "surface missing" on a host build that does
       * not expose its slot instead of blocking activation and taking the whole
       * plugin down with it.
       */
      ...
      // `sidebar.right.pane.tab` seat. `sidebarRightTabs` is provided by the
      // right-Sidebar feature's own apply, and activation order between
      // packages is not guaranteed: when this plugin runs first the service is
      // not on the ctx yet and a one-shot check would silently skip the whole
      // pane. So the registration waits for the service to appear (bounded
      // retries); a host without that surface simply never satisfies it and
      // keeps every other entry.
      var registerRightSidebar = function () {
        var sidebarRightTabs = ctx.get('sidebarRightTabs')
        if (sidebarRightTabs === undefined || typeof sidebarRightTabs.register !== 'function') return false
        ctx.effect(function () {
          return sidebarRightTabs.register({
            id: RIGHT_PANEL_ID,
            kind: RIGHT_PANEL_KIND,
            title: function () { return t('nav') },
            // Thunked copy: read fresh on every use, so a language switch needs
            // no re-registration. The icon defaults to the kit's placeholder
            // when the glyph is absent from the primitives table.
            guide: [{
              order: 90,
              title: function () { return t('nav') },
              description: function () { return t('rightSidebarDescription') },
              icon: typeof ui.IconArchiveOutline20 === 'function' ? ui.IconArchiveOutline20 : undefined,
            }],
          })
      ...
      if (!registerRightSidebar()) {
        var rightSidebarTimer = window.setInterval(function () {
          if (registerRightSidebar()) {
            window.clearInterval(rightSidebarTimer)
            window.clearTimeout(rightSidebarGiveUp)
          }
        }, 200)
        // 60 attempts ≈ 12s: enough to lose any realistic activation race, and
        // cleared with the plugin fiber so an unload stops the polling.
        var rightSidebarGiveUp = window.setTimeout(function () {
          window.clearInterval(rightSidebarTimer)
          console.warn('[dsh-better-archive] right-sidebar service never appeared; the pane entry stays unavailable')
        }, 12000)
        ctx.effect(function () {
          return function () {
            window.clearInterval(rightSidebarTimer)
        ...
        }, 'dsh-better-archive: right-sidebar wait cleanup')
      }
