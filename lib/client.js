  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && !document.getElementById(STYLE_ID)) {
      ...
      styleEl.textContent = [
        ...
        '._dsh_ba_toast_leaving { animation: _dsh-ba-toast-out .18s ease-in forwards; pointer-events: none; }',
        '@keyframes _dsh-ba-toast-out { from { opacity: 1; transform: translateX(-50%); } to { opacity: 0; transform: translate(-50%, -6px); } }',
      ].join('\n')
    ...
    function ArchivedToast(props) {
      ...
      // Two-stage dismissal, mirroring DSH's own toast: hold, then fade, then
      // unmount — so the bar leaves with a transition instead of vanishing.
      var leavingState = React.useState(false)
      var leaving = leavingState[0]
      var setLeaving = leavingState[1]
      // Held in refs: the parent re-creates its callbacks on every render, and an
      // identity dependency would restart the hold timer each time.
      var closeRef = React.useRef(props.onClose)
      closeRef.current = props.onClose

      React.useEffect(function () {
        if (hovered || leaving) return undefined
      ...
      React.useEffect(function () {
        ...
        var timer = setTimeout(function () { closeRef.current() }, TOAST_FADE_MS)
        return function () { clearTimeout(timer) }
      }, [leaving])

      return createElement('div', {
        className: leaving ? '_dsh_ba_toast _dsh_ba_toast_leaving' : '_dsh_ba_toast',
        role: 'status',
