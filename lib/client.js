  ...
  factory: (require) => {
    ...
    function ArchivedSection(props) {
      ...
      return createElement(
        'div',
        {
          style: isSidebar
            ? {
                ...
                // that height outright: the header and controls stay put and
                // the list below is the one scroller.
        ...
        isSidebar
          ? // The pane's header row, styled exactly like the main panel's title
            // row (18px title plus the delete-all outline button); it bleeds
            // over the root's side padding so its rule spans the full pane,
            // like the file tree's header.
            createElement('div', {
              style: {
                display: 'flex', flex: '0 0 auto', alignItems: 'center', justifyContent: 'space-between', gap: 12,
                boxSizing: 'border-box', minHeight: 44, padding: '8px 12px',
                margin: '0 -12px',
                borderBottom: '.5px solid var(--dsw-alias-border-l3)',
              },
            },
              createElement('h2', {
    ...
    function apply(ctx) {
      ...
      // `sidebar.right.pane.tab` seat. `sidebarRightTabs` is the right-Sidebar
      // feature's service; it is read defensively like `layout`, so a host
      // without that surface keeps every other entry and just loses this one.
      var sidebarRightTabs = ctx.get('sidebarRightTabs')
      if (sidebarRightTabs !== undefined && typeof sidebarRightTabs.register === 'function') {
        ctx.effect(function () {
