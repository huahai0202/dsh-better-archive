...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        isSidebar
          ? // Controls on one wrapping row: the search grows to a capped width
            ...
            createElement('div', {
              ...
              createElement('div', { className: '_dsh_ba_search', style: { flex: '1 1 320px', minWidth: 0, maxWidth: 440 } },
    ...
    function apply(ctx) {
      ...
      // The toast's "view" action prefers the right-Sidebar archive tab: the
      // service is read defensively like `layout`, and `openTab` throws when
      // no session surface is mounted (the column is per session), so that
      // fall-back path retargets the global panel. Without either service the
      // plugin still works; the view action just has nothing to jump to.
      ...
      if (sidebarRight !== undefined && typeof sidebarRight.openTab === 'function') {
        openArchivedPanel = function () {
          try {
            ...
            console.warn('[dsh-better-archive] right-sidebar open failed, falling back to the panel:', error)
            if (layout !== undefined && typeof layout.selectPanel === 'function') layout.selectPanel(PANEL_ID)
          }
        }
      } else if (layout !== undefined && typeof layout.selectPanel === 'function') {
