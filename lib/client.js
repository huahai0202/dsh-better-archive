...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ------------------------- i18n -------------------------
    // The panel follows the DSH language setting (Settings > General >
    // Language, exposed to client plugins as the 'locale' service) whenever
    // that service is available. English (en) and Simplified Chinese (zh) are
    // the bundled locales; English is the last-resort fallback. A manual
    // Auto / English / 中文 toggle in the panel header overrides detection
    // and persists in localStorage (Auto clears the override so the panel
    // follows the DSH setting again).
    ...
    function browserLocale() {
      try {
        var tag = String(navigator.language || '').toLowerCase()
        return tag.slice(0, 2) === 'zh' ? 'zh' : null
      } catch (e) { return null }
    }
    // Resolution chain: manual toggle override -> DSH language setting ->
    // browser language -> English.
    function resolveLang() {
      return storedOverride() || settingsLocale() || browserLocale() || 'en'
    }
    var lang = resolveLang()
    function changeLang(next) {
      // next: 'auto' (follow the DSH setting), 'en', or 'zh'
      try {
        if (next === 'auto') window.localStorage.removeItem(LANG_STORAGE_KEY)
        else window.localStorage.setItem(LANG_STORAGE_KEY, next)
      } catch (e) { /* ignore */ }
      lang = resolveLang()
    }
    function t(key, arg) {
    ...
    function LangToggle(props) {
      var options = [
        { value: 'auto', label: 'Auto', title: 'Follow the DSH language setting' },
    ...
    function ArchivedSection(props) {
      ...
      var langState = React.useState(resolveLang())
      var uiLang = langState[0]
      var setUiLang = langState[1]
      function onLangChange(next) {
        changeLang(next)
        setUiLang(resolveLang())
      }
      // Follow the DSH language setting live while no manual override is set;
      // with the locale service absent this is a no-op and the panel keeps the
      // stored/browser/English fallback chain.
      React.useEffect(function () {
        var svc = localeSvc
        if (svc === null || typeof svc.subscribe !== 'function') return undefined
        return svc.subscribe(function () {
          if (storedOverride()) return
          var next = resolveLang()
          if (next !== lang) {
            lang = next
            setUiLang(next)
          }
      ...
      return createElement(
        ...
        createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 } },
          ...
          createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
            createElement(LangToggle, { value: storedOverride() || 'auto', onChange: onLangChange }),
            createElement('button', {
              onClick: deleteAll,
    ...
    function apply(ctx) {
      ...
      // Adopt the DSH locale service when the running GUI provides one; the
      // resolution chain falls back to stored/browser/English otherwise.
      try {
        var svc = ctx.get('locale')
        if (svc !== null && typeof svc === 'object' && typeof svc.getLocale === 'function') {
          localeSvc = svc
          lang = resolveLang()
        }
      } catch (e) { /* locale service unavailable */ }
