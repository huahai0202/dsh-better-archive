...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zhDict = {
      ...
      confirmDeleteBody: '删除的聊天会移入「已删除」区域；可以点「恢复」找回，永久删除后将无法恢复。',
      restartDelete: '将永久删除',
      deletedLabel: '已删除',
      deletedSection: '已删除',
      restore: '恢复',
      scheduledNotice: '{count} 个会话已移入「已删除」区域。',
      pendingLoadFailed: '无法读取待删除状态，请稍后重试。',
      cancel: '取消',
      unarchivedNotice: '对话已取消归档。',
      view: '查看',
      dismiss: '关闭',
    }
    ...
    var enDict = {
      ...
      confirmDeleteBody: 'Deleted chats move to the Deleted section. Restore brings them back; once permanently deleted they cannot be recovered.',
      restartDelete: 'Will be permanently deleted',
      deletedLabel: 'Deleted',
      deletedSection: 'Deleted',
      restore: 'Restore',
      scheduledNotice: '{count} session(s) moved to the Deleted section.',
      pendingLoadFailed: 'Unable to load pending deletion status. Try again later.',
      cancel: 'Cancel',
      unarchivedNotice: 'Chat unarchived.',
      view: 'View',
      dismiss: 'Dismiss',
    }
    ...
    function ArchivedSection(props) {
      ...
      // Display-only split: sessions marked for deletion after restart are
      // shown as "deleted" (greyed, struck-through, in their own section).
      // The underlying pending-deletion flow is unchanged — they remain
      // archived rows until the host physically removes them.
      var activeRows = []
      var deletedRows = []
      rows.forEach(function (row) {
        if (row.pendingDeletion) deletedRows.push(row)
      ...
      groupKeys.forEach(function (key) {
        var members = activeRows.filter(function (row) { return (row.cwd || '') === key })
      ...
      function unarchiveOne(row) {
        return act('/archived/unarchive', { sessionId: row.id }).then(function (res) {
          if (res) setNotice({ kind: 'unarchived', sessionId: row.id })
          return res
        })
      }
      function viewUnarchivedSession() {
        if (!notice || notice.kind !== 'unarchived') return
        props.sessions.open(notice.sessionId)
      ...
      // One row of the archived list. A row whose deletion is already
      // scheduled renders as "deleted": greyed, struck-through title, a
      // "deleted" badge, no delete button; its only action is restoring
      // (unarchive), which — through the unchanged host logic — also cancels
      // the pending-deletion marker.
      function renderRow(row) {
        var deleted = row.pendingDeletion
        ...
        return createElement(
          ...
          createElement('div', { style: { flex: '1 1 auto', minWidth: 0 } },
            createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 } },
              ...
              deleted ? createElement('span', {
                title: tr('restartDelete'),
                style: {
                  flex: '0 0 auto', padding: '2px 6px', borderRadius: 6, fontSize: 11,
                  color: dark ? '#e89a9a' : '#a94040',
                  background: dark ? 'rgba(229,83,75,0.12)' : 'rgba(220,38,38,0.08)',
                  border: '1px solid ' + (dark ? 'rgba(229,83,75,0.30)' : 'rgba(169,64,64,0.25)'),
                },
              }, tr('deletedLabel')) : null,
            ),
            shownTime ? createElement('div', { style: { fontSize: 12, opacity: 0.6, marginTop: 3 } }, formatDate(shownTime)) : null,
          ),
          deleted ? null : createElement('button', {
            onClick: function () { deleteOne(row) },
            disabled: busy,
            title: tr('delete'),
            style: { ...btnStyle, background: 'transparent', color: dark ? 'rgba(255,255,255,0.65)' : 'rgba(31,35,40,0.58)', padding: '4px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
          }, TrashIcon()),
          createElement('button', {
            onClick: function () { unarchiveOne(row) },
            disabled: busy,
            title: deleted ? tr('restartDelete') : undefined,
            style: { ...btnStyle, padding: '7px 11px', fontSize: 13, fontWeight: 500, background: 'rgba(128,128,128,0.16)' },
          }, deleted ? tr('restore') : tr('unarchive')),
        )
      }
      ...
      return createElement(
        ...
        notice ? (notice.kind === 'unarchived'
          ? createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, color: dark ? '#8bd5a7' : '#18794e' } },
              createElement('span', null, tr('unarchivedNotice')),
              createElement('button', { type: 'button', onClick: viewUnarchivedSession, style: { ...btnStyle, padding: '4px 8px', color: 'inherit', fontWeight: 600 } }, tr('view')),
              createElement('button', { type: 'button', onClick: function () { setNotice('') }, 'aria-label': tr('dismiss'), style: { border: 'none', background: 'transparent', color: 'inherit', cursor: 'pointer', padding: 2, fontSize: 14 } }, '\u00d7')
            )
          : createElement('p', { style: { fontSize: 12, color: dark ? '#8bd5a7' : '#18794e', margin: 0 } }, notice)) : null,
        ...
        allRows.length === 0
          ...
          : groups.length === 0 && deletedRows.length === 0
            ...
            : createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 20, maxHeight: '58vh', overflowY: 'auto', paddingRight: 2 } },
                ...
                deletedRows.length > 0 ? createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
                  createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, minHeight: 22, fontSize: 14, color: dark ? 'rgba(255,255,255,0.55)' : 'rgba(31,35,40,0.55)' } },
                    TrashIcon(),
                    createElement('span', { style: { flex: '1 1 auto', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 600 } }, tr('deletedSection')),
                    createElement('span', { style: { marginLeft: 'auto', fontSize: 13, color: dark ? 'rgba(255,255,255,0.5)' : 'rgba(31,35,40,0.45)' } }, tr('chatCount', { count: deletedRows.length })),
                  ),
                  createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
