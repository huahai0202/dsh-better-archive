...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && !document.getElementById(STYLE_ID)) {
      ...
      styleEl.textContent = [
        '._dsh_ba_menu_root { width: 100%; display: flex !important; }',
    ...
    function getPrimitive(name) {
      return (primitives && primitives[name]) ? primitives[name] : null
    }

    function TrashIcon(props) {
      var Comp = getPrimitive('IconTrashOutline16')
    ...
    function SearchIcon(props) {
      var Comp = getPrimitive('IconSearchOutline16')
      if (Comp) return createElement(Comp, props)
      var size = (props && props.size) || 16
      return createElement('svg', {
        width: size, height: size, viewBox: '0 0 16 16', fill: 'none', xmlns: 'http://www.w3.org/2000/svg',
    ...
    function ChevronDownIcon(props) {
      var Comp = getPrimitive('IconChevronDownOutline14')
      if (Comp) return createElement(Comp, props)
    ...
    function FolderIcon(props) {
      var Comp = getPrimitive('IconFolderClose16')
      if (Comp) return createElement(Comp, props)
      var size = (props && props.size) || 16
      return createElement('svg', {
        width: size, height: size, viewBox: '0 0 16 16', fill: 'none', xmlns: 'http://www.w3.org/2000/svg',
    ...
    function EllipsisIcon(props) {
      var Comp = getPrimitive('IconEllipsisOutline16')
      if (Comp) return createElement(Comp, props)
      var size = (props && props.size) || 16
      return createElement('svg', {
        width: size, height: size, viewBox: '0 0 16 16', fill: 'none', xmlns: 'http://www.w3.org/2000/svg',
    ...
    function CheckIcon(props) {
      var Comp = getPrimitive('IconCheckOutline16')
      if (Comp) return createElement(Comp, props)
      var size = (props && props.size) || 16
      return createElement('svg', {
        width: size, height: size, viewBox: '0 0 16 16', fill: 'none', xmlns: 'http://www.w3.org/2000/svg',
    ...
    function Dropdown(props) {
      ...
      var menuItems = []
      if (props.header) {
        menuItems.push({ type: 'label', id: '__header__', text: props.header })
      }
      props.options.forEach(function (option) {
        menuItems.push({ id: option.value, label: option.label })
      })

      if (primitives && primitives.Menu) {
        return createElement('div', {
          style: { position: 'relative', width: '100%', ...(props.containerStyle || {}) },
        },
          createElement(primitives.Menu, {
            open: open,
    ...
    function GroupMenu(props) {
      ...
      if (primitives && primitives.Menu) {
        return createElement(primitives.Menu, {
          open: open,
          onClose: function () { setOpen(false) },
          items: [
            {
              id: 'delete-project',
              label: t('deleteProjectContent'),
          ...
          onSelect: function (id) {
            setOpen(false)
            if (id === 'delete-project') props.onDelete()
          },
          align: 'end',
          portal: true,
          anchor: trigger,
    ...
    function ArchivedSection(props) {
      ...
      return createElement(
        ...
        pendingConfirm ? (
          primitives && primitives.Modal
            ? createElement(primitives.Modal, {
                open: Boolean(pendingConfirm),
                onClose: cancelDelete,
                title: tr('confirmDeleteTitle'),
                closeLabel: tr('cancel'),
                description: tr('confirmDeleteBody'),
                footer: createElement(React.Fragment, null,
                  createElement('button', {
                    type: 'button',
