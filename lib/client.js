  ...
  factory: (require) => {
    ...
    function ArchivedSection(props) {
      ...
      return createElement(
        ...
        isSidebar
          ? // The pane's 38px header row, like the file tree's: the tab chip
            // already names the page, so this row just states it and offers the
            // one destructive action, and the list owns everything below.
            createElement('div', {
              style: {
                display: 'flex', flex: '0 0 auto', alignItems: 'center', gap: 8,
                boxSizing: 'border-box', height: 38,
                // Bleed over the root's side padding so the row and its rule
                // span the full pane, like the file tree's header, while its
                // content lines up with the 12px content inset below.
                margin: '0 -12px', padding: '0 12px',
                borderBottom: '.5px solid var(--dsw-alias-border-l3)',
              },
            },
              createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flex: '1 1 auto', minWidth: 0 } },
                createElement(Icon, { name: 'IconArchiveOutline20', size: 16 }),
                createElement('span', {
                  style: {
                    minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                    fontSize: 13, lineHeight: '22px', fontWeight: 600,
                    color: 'var(--dsw-alias-label-primary, inherit)',
                  },
                }, tr('nav')),
              ),
