...
async function removeProjectionCacheRow(ctx, id) {
  try {
    ...
    console.warn('[better-archive] projection cache cleanup failed:', String(error))
  }
}
...
async function detachFromWorkspace(ctx, id) {
  try {
    ...
    console.warn('[better-archive] workspace detach failed:', String(error))
  }
}
...
async function deleteSession(ctx, id) {
  ...
  if (live !== undefined) {
    try {
      ...
      console.warn('[better-archive] live session release failed:', String(error))
    }
  }
...
function apply(ctx) {
  ctx.effect(() => ctx.webServer.register({
    ...
    }), 'better-archive: /archived/list route')
    ...
    ctx.effect(() => ctx.webServer.tapIndex((html) =>
      html.replace('</head>', `<meta name="dsh-better-archive" content="${nonce}">\n</head>`),
    ), 'better-archive: index nonce')
